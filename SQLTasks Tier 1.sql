...
CASE WHEN monthlymaintenance > 100 THEN 'expensive' 
        ELSE 'cheap' END AS reslative_expense 
FROM Facilities
...
SELECT * FROM (SELECT f.name, CONCAT(surname, ' ', firstname) AS membername,
CASE 
    WHEN memid !=0 THEN (membercost * slots)
    ELSE guestcost * slots
    END AS cost
...
WHERE starttime LIKE '2012-09-14'
AND memid !=0 AND (membercost * slots) >30
OR memid =0 AND (guestcost * slots) >30) AS sub
...
FROM (SELECT f.name, CONCAT(surname, ' ', firstname) AS membername, CASE
        WHEN memid !=0 THEN (membercost * slots)
        ELSE guestcost * slots
        END AS cost
FROM Facilities AS f
